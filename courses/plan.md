...
| M04 | Ch2 Decomposition | 🔵 | §1 `02-decomposition/01-cohesion-coupling-and-module-depth.md` 🔵 (drafted 2026-06-18) | **§1 drafted 2026-06-18** — the conceptual core of his clearest gap. Rotated here off 4 straight M01 days, per the standing note that M04 Ch2 is "the high-value next SWE stop, should use his pipeline code." Body: kills the "decomposition = smaller files" metric → complexity (change amplification / cognitive load / unknown-unknowns) → cohesion ladder → coupling ladder + the **decomposition U-curve** (xychart plot: total = within-module ↓ + between-module ↑, valley ≠ line count) → **module depth** as the unifying idea ($\text{depth}\approx\text{functionality}/\text{interface}$) → re-diagnoses his **`PipeState` `update_state(data)` as a maximally shallow / negative-depth interface** (M01 Ch2 §1 §10 callback) → info hiding & leaky abstractions (ties his "define errors out of existence" keeper) → when NOT to decompose. Two figures (U-curve + ❌shared-bag vs ✅data-flow structures). §10 hands-on = paper-decompose `process_no_waiting.py`. Awaiting his Q&A → finalize. Next inside Ch2: §2 refactoring-a-monolith-in-moves, or §3 module/file boundaries; or rotate to M12 Ch2 §2 video. |
