...
| M01 | Ch3 Concurrency | 🔵 | §1 `03-processes-threads-concurrency/01-concurrency-parallelism-and-the-gil.md` ✅ (2026-06-16); §2 `03-processes-threads-concurrency/02-async-deeply.md` 🔵 DRAFT (2026-06-25) | **§2 (async deeply) — DRAFT 2026-06-25, awaiting Q&A + finalize.** One level under §1, cashing the 9b timeout keeper into its mechanism. Body: **(§1)** the event loop one tick at a time — `_ready` deque + `_scheduled` heap + selector; the single blocking `selector.select()`/`epoll_wait`; timer-derived timeout; the `ntodo` fairness snapshot. **(§2)** coroutine vs Future vs Task (table) + the await→park→wake cycle at the `send`/`throw` level (Task.__step driving the coroutine); `await coro` ≠ a Task → two awaits are serial. **(§3)** spawning primitives table (`await`/`create_task`/`gather`/`as_completed`/`wait`/`TaskGroup`) keyed on *sibling-on-error* behaviour; gather's default leaves siblings orphaned; `return_exceptions=True`=harvest; as_completed=stream/persist. **(§4)** structured concurrency — go-statement-considered-harmful (Smith/Trio nurseries), `TaskGroup`+`ExceptionGroup`/`except*` (PEP 654), fail-fast vs harvest. **(§5, centrepiece)** cancellation = an exception *injected* (`coro.throw(CancelledError)`) at the parked `await`; only lands at an await (so un-yielding code is un-cancellable, same root as blocking-the-loop); *why* timeout beats silence but `return_exceptions` can't; `CancelledError`∈BaseException → must re-raise; shield/finally/uncancel. **(§6)** footgun gallery (fire-and-forget GC + set/discard idiom; gather orphans; swallowed CancelledError; blocking the loop; unretrieved exceptions; `asyncio.run` nesting/Jupyter). 3 Mermaid diagrams (tick cycle; await→park→wake sequence; cancellation-injection sequence) rendered + visually verified. Refs verified live (asyncio task/eventloop/dev docs, Smith blog, Trio, PEP 654/3156/492). §8 check-understanding, §9 hands-on (4 runnable snippets incl. the timeout-can't-cancel-CPU demo), §10 Applied placeholder. **§1 finalized 2026-06-16.** Body (two orthogonal axes via Pike + a 2×2 *table* — Mermaid couldn't lay out the grid; multiplexing framing · processes/threads/async cost table · the GIL pinned down: what it locks, ~5ms switch interval, releases on I/O + in good C extensions → "threads parallelize only GIL-released work" · CPU-vs-IO decision + `run_in_executor` hybrid · free-threading/PEP 703) held at his level and went **untouched** — he drove the whole session **applying** it to an **LLM-eval pipeline**. §9 (Applied) captures three threads: **(9a)** "batch" = 3 mechanisms (client fan-out / Batch API / continuous batching); the `openai` chat "batch" he uses *is* §1–§4 async fan-out, ceiling = rate limit not GIL (06-09 callback), ties to his 06-15 continuous-batching reading. **(9b)** `asyncio.gather` failure mode — scheduler model right; **mis-ranked the failure** by fusing two opposite hangs: a task *parked on I/O* doesn't hang the thread (only `gather`'s join barrier waits; 999 results recoverable) vs a *blocking call* that freezes the loop (§4 footgun in parse/aggregate). Keeper: `return_exceptions` handles **errors**, only a **timeout** handles **silence** → for "response never comes," timeout is the dominant fix (Ousterhout "define errors out of existence"). **(9c)** parse/calc — lever is *push the loop into C, not thread-vs-process*; **"C library" ≠ "GIL released"** (numpy releases→parallel; json/orjson/pydantic hold→fast single-thread; lxml releases); the ladder (don't bother→C/Rust lib→ProcessPool last). Next inside Ch3: §2 Async-deeply (cancellation — cashes 9b) or §3 sync/races; or rotate scope (4th straight M01 day) to M04/M12. |
| M01 | Ch4 I/O & kernel | ⬜ | | |
