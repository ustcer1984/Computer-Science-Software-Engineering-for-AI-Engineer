...
| M01 | Ch3 Concurrency | 🔵 | §1 `03-processes-threads-concurrency/01-concurrency-parallelism-and-the-gil.md` 🔵 DRAFT (2026-06-16) | §1 (concurrency vs parallelism + the three models + the GIL) **drafted 2026-06-16**, pending Q&A/finalize. Pitched to *cash in* the GIL keystone (Ch2 §2 refcounting→GIL) and the async-stack model (Ch1 §2) rather than re-teach them. Body: the two orthogonal axes (Pike, the 2×2, multiplexing framing); processes/threads/async cost-model table; the GIL pinned down (what it locks, ~5ms switch interval, releases on I/O + in C extensions → the one rule); CPU-vs-IO decision flowchart + the `run_in_executor(process_pool)` hybrid; free-threading/PEP 703 as the keystone paid off. Hooks his arena async audit (serial `await`s → `gather`; blocking calls on the loop). §11 Applied = placeholder. Refs verified live. |
| M01 | Ch4 I/O & kernel | ⬜ | | |
