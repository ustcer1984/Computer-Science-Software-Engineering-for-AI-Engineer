...
- **Post-training already grades by outcome, not loss.** RLHF/DPO — reinforcement learning from human feedback, and Direct Preference Optimization — /**RLVR (reinforcement learning from verifiable rewards)** optimize a reward/verifier, not token cross-entropy — that's "judge by the exam," and it's your own [16-rl-verifiers reading](../06/16-rl-verifiers-and-environments.md). The field is half-way to your world already.
...
- **Fine-grained curriculum learning mostly does *not* beat a shuffled mix at scale** (well-shuffled IID — independent and identically distributed — batches are a brutally strong baseline; "difficulty" is ill-defined for text). Wins survive only at the *phase* granularity, not per-example. Your "grade by grade" is right at the granularity of *terms*, not *lessons*.
...
- **Goodhart** is *the* reason interp isn't already a training target: make "look monosemantic to the SAE (sparse autoencoder)" an objective and SGD fakes it. Gating (a discrete promote/hold) is the Goodhart-*milder* use, which is why your framing survives where a naive interp-loss wouldn't.
...
- [First imagery from NSF (National Science Foundation)–DOE (Department of Energy) Vera C. Rubin Observatory](https://rubinobservatory.org/news/first-imagery-rubin)
