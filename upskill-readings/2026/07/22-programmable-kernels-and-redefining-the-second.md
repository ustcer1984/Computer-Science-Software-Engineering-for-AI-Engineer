...
**D. The real culprit was one number: an uncapped power limit.** RAPL showed **PL1 = 115 W** on a chip whose *nominal* TDP is 28 W — so nothing but temperature governs the boost. The CPU ramps to 5 GHz, the favored core slams into 110 °C, hardware hard-throttles it to \~2.5 GHz, it cools, it ramps again — and **that boost↔throttle oscillation *is* the stutter** (a frame that should take 6 ms lands mid-throttle and takes 16 ms). `thermald`, which would smooth this, was off. The counter-intuitive keeper: **capping the CPU can make the game smoother** — a steady 4 GHz beats a bouncing 5.0↔2.5. Ranked fixes: enable `thermald` → cap PL1 / max frequency → cap in-game FPS → repaste (PTM7950 on these ThinkPads) to raise the ceiling. `sched_ext` lands *last* for this problem — it cannot add cooling; its one honest contribution on a hybrid chip is *placement* (keep the hot thread on a P-core, don't stack its SMT sibling), a safe experiment (`scx_bpfland`, and the kernel falls back if it misbehaves) worth trying only *after* the thermal fixes.
