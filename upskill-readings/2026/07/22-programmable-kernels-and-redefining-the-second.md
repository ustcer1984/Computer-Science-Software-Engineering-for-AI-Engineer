...
2. ⏱️✨ **We're about to redefine what a second *is* — and the new one is built on light.** Since 1967 the second has been defined by a microwave hum inside a caesium atom: exactly 9,192,631,770 oscillations. That definition is now the *slowest* part of our best clocks. A new generation of **optical** atomic clocks — probing a transition that buzzes not billions but *hundreds of trillions* of times a second — has blown past caesium by a factor of about 100. In **July 2025** a clock at NIST (National Institute of Standards and Technology) built on a single trapped aluminum ion hit **19 decimal places** of accuracy: it wouldn't gain or lose a second in *roughly 40 billion years*, several times the age of the universe. The world's metrologists have a roadmap to **redefine the SI second on optical clocks around 2030** — and, in the same breath, to **abolish the leap second by 2035**.
...
> **The "huh, I didn't know that" file.** Two twists most people miss. **First — redefining the second is a committee marathon, not a switch-flip.** The plan (endorsed by the international timekeeping bodies) is to pick *which* optical transition — or *combination* of them — becomes the new definition, satisfy a checklist of "mandatory criteria" (multiple independent clocks worldwide must agree, and the ultra-precise links to compare them across continents must be proven) by about 2029, and have the General Conference on Weights and Measures **ratify a new second around 2030**, with 2034 as a backup. There's even a serious proposal to define the second not on one atom but on the *geometric mean of several* optical transitions at once, hedging against any single one turning out imperfect. **Second — the same era is killing the leap second.** Because Earth's rotation is slightly irregular, since 1972 we've occasionally inserted a "leap second" into UTC (Coordinated Universal Time) to keep atomic time aligned with the spinning planet — a nightmare for computers (leap seconds have crashed Reddit, Cloudflare and airline systems). In 2022 the world's metrologists **voted to abolish the leap second by 2035**, letting atomic time and Earth-rotation time drift apart by more than a second before anyone bothers to reconcile them. So within about a decade, humanity will have *both* a new, light-based definition of the second *and* a decision to stop chaining our clocks to the wobble of the Earth. The tick is being cut loose from the planet.

---

## What we worked out — the thread you drove (read this first on review)

Story 2 (the clocks) stayed a read this time. But Story 1 didn't just spark a chat — it turned into a **live hardware debug of your own machine**: a ThinkPad on an **Intel Core Ultra 7 165H (Meteor Lake)** where a Wine game lags intermittently, always coinciding with **one CPU core near 100% while the rest idle**. That symptom is a great teacher, because it's a trap.

**A. The reframe: one-core-at-100%-others-idle is a *serial-bottleneck* signature, not a scheduling-imbalance one.** A scheduler picks *which* core a thread runs on and *when* — it can never split one thread across cores. So before reaching for `sched_ext`, the fork is: is a *ready* thread being starved (scheduler's job) or is one thread genuinely needing more than a core (nothing scheduling can fix)? The reading's own caveat — the verifier proves a scheduler *safe*, not *good* — is the same lesson from the other side.

**B. You localized it empirically to *thermal*, and the hardware confirmed it.** Your method was the right one: Psensor showed >90 °C, you found dust, cleaned it, temps fell to 70–80 °C and the lag *dropped* — a clean natural experiment pointing at cooling. Reading the machine's own counters closed the case: the per-core **thermal-throttle counters** (`/sys/.../thermal_throttle/core_throttle_count`) read **\~2300 on exactly two cores — the favored 5.0 GHz P-cores — and zero everywhere else.** (That also explains "package only 80 °C but still lags": the package sensor is an *average*; one favored core sits near its 110 °C limit while 21 cool cores drag the mean down.)

**C. Your proposed fix — "migrate the thread to a cooler core" — is the *weakest* lever (the correction).** Three reasons, in rank order: (1) it's a **laptop** — all six P-cores share one die, heatpipe and fan, so a *sustained* load reheats a migrated core to the same steady state in about 10 ms; you can't rotate your way out of a shared thermal budget. (2) The cool cores are the **slow** cores (4.7 GHz P-cores, or 3.8 GHz E-cores), and your bottleneck is single-threaded — so you'd trade throttle-stutter for lower baseline speed. (3) The scheduler and hardware already rotate the favored core somewhat. Rotation only helps *bursty* peaks, never a sustained cap.

**D. The real culprit was one number: an uncapped power limit.** RAPL (Running Average Power Limit) showed **PL1 = 115 W** on a chip whose *nominal* TDP (thermal design power) is 28 W — so nothing but temperature governs the boost. The CPU ramps to 5 GHz, the favored core slams into 110 °C, hardware hard-throttles it to \~2.5 GHz, it cools, it ramps again — and **that boost↔throttle oscillation *is* the stutter** (a frame that should take 6 ms lands mid-throttle and takes 16 ms). `thermald`, which would smooth this, was off. The counter-intuitive keeper: **capping the CPU can make the game smoother** — a steady 4 GHz beats a bouncing 5.0↔2.5. Ranked fixes: enable `thermald` → cap PL1 / max frequency → cap in-game FPS (frames per second) → repaste (PTM7950 on these ThinkPads) to raise the ceiling. `sched_ext` lands *last* for this problem — it cannot add cooling; its one honest contribution on a hybrid chip is *placement* (keep the hot thread on a P-core, don't stack its SMT — simultaneous multithreading — sibling), a safe experiment (`scx_bpfland`, and the kernel falls back if it misbehaves) worth trying only *after* the thermal fixes.

**E. The measurement rig you'll carry forward.** The throttle counter is a built-in instrument: record it, play 15 minutes, read the **delta** = throttle events that session; add **MangoHud** for the frametime graph; then change one thing at a time and watch the delta head toward zero. (Full diagnosis + commands live in a local handoff note; you're taking the fix into a separate project.)

> **Where we landed.** Your diagnosis was right (thermal throttling) and your *method* was exemplary (measure, perturb cooling, re-measure). The one correction was a **leverage ranking**: core choice is the weakest knob; the binding constraints are the laptop's shared cooling and an uncapped power limit, and the stutter is specifically the *throttle-recovery oscillation*. `sched_ext` was the shiny tool the reading put in your hand, and the honest engineering answer is that it's **not** the tool for this job — which is itself the more useful thing to know.
...
*Finalized 2026-07-26 (created 2026-07-22) — two feature stories in the "Nat-Geo / Discovery" register: one **career-track** (eBPF — safe, verified, JIT-compiled programs loaded into a running Linux kernel; the `sched_ext` pluggable-scheduler frontier) and one **hobby-track** (the coming redefinition of the SI second on optical atomic clocks, and the abolition of the leap second by 2035). Figures current to mid-2026. The **"What we worked out"** section is the durable record — read it first on review: Story 2 (clocks) stayed a read, but Story 1 became a **live hardware debug** of his own **Meteor Lake ThinkPad** (a Wine game lagging whenever one core pegs 100%). He drove one strong thread — correctly localizing the lag to **per-core thermal throttling** by good empirical method (dust-clean natural experiment), confirmed via hardware introspection (throttle counters \~2300 on the favored 5.0 GHz P-cores; an uncapped **PL1 = 115 W** on a 28 W-nominal chip → the boost↔throttle **oscillation** is the stutter). The one correction was a **leverage ranking**: his "migrate to a cooler core" remedy is the weakest knob (shared laptop cooling; cool cores are the slow cores), and `sched_ext` — the reading's shiny tool — is honestly **not** the fix here (it can't add cooling; it's a placement-only, safe experiment for later). Real levers: `thermald` → cap PL1/frequency (cap-to-go-faster) → FPS cap → repaste. Visuals: 2 ComfyUI path-4 illustrations (a sandboxed program-creature in the kernel engine room; a trapped ion in a web of laser light) + 1 matplotlib figure (clock fractional uncertainty vs year, caesium floor vs the optical dive to 10⁻¹⁹) + 1 Mermaid diagram (old kernel-change paths vs the eBPF verify→JIT→attach flow).*
