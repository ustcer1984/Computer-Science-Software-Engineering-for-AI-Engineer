...
**B. Your call — diffusion's speed is a *single-user* win, not multi-user-efficient. (Correct.)** Roofline: AR at batch-1 is **bandwidth-bound** (idle FLOPs) → batching fills the FLOPs → throughput scales \~linearly. A single diffusion request **already sits near the compute roof** → it *pre-spends* the exact FLOP headroom AR banks for concurrency. So it's a **latency-at-low-load** win; at saturation it does more compute per token ($\sim S \cdot 2N$ vs AR's $\sim 2N$) → worse throughput per dollar. The twist to hold: diffusion is **memory-light** (little/no KV cache), so it can *pack* more users (memory-wise) but not *accelerate* them — the binding constraint flips from **HBM (AR)** to **FLOPs (diffusion)**. The real serving choice is a **crossover in the (load, latency-SLO) plane**, not a flat winner.
